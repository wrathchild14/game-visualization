...
let points_g = [],
    games_data_g = [];
let multiplier_diagram_x_g = 1 / 4,
    multiplier_diagram_y_g = 1 / 2,
    multiplier_r_g = 0; // inital multiplier values
...
function draw() {
    ...
    const diagramX = multiplier_diagram_x_g * width;
    const diagramY = multiplier_diagram_y_g * height;
    const radius = multiplier_r_g * width;

    const angle = 360 / rows;

    const min_value = -5.09;
    const max_value = 82.53;

    for (let i = 0; i < rows; i += 1) {
        // drawing the lines
        const line_size = map(games_data_g[i].global_sale, min_value, max_value, 0, 205);
        const pointX = (line_size + radius) * cos(angle * i) + diagramX;
        const pointY = (line_size + radius) * sin(angle * i) + diagramY;
        points_g.push([pointX, pointY]);
        const cirx = radius * cos(angle * i) + diagramX;
        const ciry = radius * sin(angle * i) + diagramY;

        // every 10 lines make it thicker
        if (i % 10 == 0) {
            push();
...
function show_sub_text(name, global_sale, user_scores, user_counts) {
    textSize(16);
    textAlign(LEFT);
    fill("black");
    if (global_sale !== undefined && user_counts !== "") {
        text(
            `${name} has ${global_sale} million global sales with a rating of ${user_scores} from ${user_counts} users.`,
            width / 4,
            height / 4,
            width / 3 + 140
        );
...
function show_focused_text(fade_g) {
    ...
    textSize(16);
    text(
        `${game.title} was created by ${game.developer} in ${game.release_year} on the platform ${game.platform}. It had a total of ${game.global_sale} million sales with a user rating of ${game.user_score} from ${game.user_count} users.`,
        width / 2,
        height / 3 + 150,
        width / 3 + 100
    );
...
function handle_drawing_point(pointX, pointY, diagramX, diagramY, focused_point) {
    ...
    if (dis < 5) {
        ...
        if (!zoomed_state_g) {
            ...
            show_sub_text(games_data_g[i].title, games_data_g[i].global_sale, games_data_g[i].user_score, games_data_g[i].user_count);
        }
    } else {
        fill(1, 73, 1, 255);
        pointSize = 4;
...
function setup_data(global_sales_minimum) {
